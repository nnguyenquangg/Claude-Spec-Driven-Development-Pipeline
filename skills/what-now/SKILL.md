...
🗺️  CHỌN ĐƯỜNG THEO LOẠI VIỆC
  ...
  • Feature / thay đổi → /make-plan …    (đường SDD 2 phase bên dưới)
...
- Stage 0 → `/make-plan "<điều bạn muốn>"` (lên specs + ADR cho review)
...
- `/fix <bug>` — **đường nhẹ cho bug**: diagnose → chọn expert → minimal fix → verify; tự leo thang sang /make-plan nếu hoá ra to
- `/make-plan <mô tả>` — **Phase 1**: clarify → specs + ADR → recommend expert → DỪNG cho review (không code)
