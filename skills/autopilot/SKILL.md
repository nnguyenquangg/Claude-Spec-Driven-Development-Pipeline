...
Run `spec-loop` for the change, implementing **as the chosen experts** (load their guidance, or delegate chunks to them as sub-agents for larger work). spec-loop loops implement → independent review vs the finalized specs + ADRs → fix → repeat (cap 6). Then run the project's quality gate (lint/build/test as available; respect host CLAUDE.md — e.g. don't run `tsc`/`prisma` yourself here).
